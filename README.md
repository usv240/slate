...
2. schedule burn is positive across at least two consecutive evaluation windows;
3. work remains.
...
- `11 passed` locally, including healthy and poisoned-input real FFmpeg integration tests.
...
- Three-role Google ADK topology using Gemini on Vertex AI.
- Grafana MCP client that fails closed without `GRAFANA_MCP_COMMAND`.
- Public API: `/v1/deliveries`, `/v1/jeopardy/{id}`, remediation approval, and `/metrics`.
