...
def test_the_page_and_the_end_to_end_checker_agree_on_the_window():
    ...
    # The README describes the proof to a reader who has not run it, so it is
    # part of the claim. The demo script and the Devpost copy said it too, and
    # both went stale once; they now live outside the repository, where this
    # check cannot reach them.
    spoken = {"40": "forty", "50": "fifty", "55": "fifty-five", "60": "sixty"}[str(int(window))]
    text = (root / "README.md").read_text(encoding="utf-8").lower()
    assert f"{spoken} second" in text, "README.md names a different window"
