...
- [x] `scripts/mutation_check.py` breaks the classifier six ways in CI and fails
      ...
      straight through it, and the guard reported success with the leak present
...
- [x] Demo-video panel that does not exist until there is a video: it ships hidden, the
      console row runs full width without it, and setting `SLATE_DEMO_VIDEO_URL` reveals it
      as a nocookie embed. An empty slot announcing that nothing is recorded reads as
      unfinished on the one surface a judge lands on first
